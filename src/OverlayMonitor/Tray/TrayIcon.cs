...
public sealed record TrayMenuState(bool Visible, bool Moving, bool AutoStart, OverlayProfile Profile, OverlayTheme Theme);
...
public sealed class TrayIcon : IDisposable
{
    ...
    private void ShowMenu() { var state = StateProvider?.Invoke() ?? new(true, false, false, OverlayProfile.Custom, OverlayTheme.AdaptiveOutline); var menu = NativeMethods.CreatePopupMenu(); try { Add(menu, 1, "显示/隐藏", state.Visible); Add(menu, 2, "移动模式", state.Moving); AddSeparator(menu); Add(menu, 3, "NVIDIA 补充模式", state.Profile == OverlayProfile.Supplement); Add(menu, 4, "完整模式", state.Profile == OverlayProfile.Full); Add(menu, 5, "温度模式", state.Profile == OverlayProfile.Temperature); AddSeparator(menu); Add(menu, 9, "主题：自适应描边", state.Theme == OverlayTheme.AdaptiveOutline); Add(menu, 10, "主题：轻量底板", state.Theme == OverlayTheme.OriginalWhite); AddSeparator(menu); Add(menu, 6, "开机自启动", state.AutoStart); Add(menu, 7, "重新加载配置"); Add(menu, 8, "退出"); AddSeparator(menu); AddDisabled(menu, $"OverlayMonitor v{GetVersion()}"); NativeMethods.GetCursorPos(out var p); NativeMethods.SetForegroundWindow(_data.hWnd); var id = NativeMethods.TrackPopupMenu(menu, 0x100, p.x, p.y, 0, _data.hWnd, 0); if (id != 0) Command?.Invoke(id); NativeMethods.PostMessage(_data.hWnd, NativeMethods.WM_NULL, 0, 0); } finally { NativeMethods.DestroyMenu(menu); } }
